...
func TestTrackedLocalAdoptMessagesIgnoresLegacyMailboxLabels(t *testing.T) {
	root := t.TempDir()
	a := &config.Account{
		Name:      "acct",
		Protocol:  "jmap",
	...
	s := state.New()
	s.Entries["legacy-state-key"] = state.Entry{
		Protocol:      "jmap",
		RemoteMailbox: "Inbox",
		RemoteID:      "server-id",
		LocalMailbox:  "Inbox",
	...
	if got[0].FileKey != key {
		t.Fatalf("FileKey = %q, want %q", got[0].FileKey, key)
	}
}

func TestReconcileLegacyDuplicateWithMismatchedStateLabels(t *testing.T) {
	root := t.TempDir()
	stateDir := t.TempDir()
	a := &config.Account{
		Name:      "acct",
